...
abstract class PhabricatorApplication
  ...
  final public static function getApplicationGroups() {
    return array(
      ...
      self::GROUP_DEVELOPER     => pht('Các công cụ phát triển'),
    );
  }
...
/* -(  Application Information  )-------------------------------------------- */
  ...
  public function getShortDescription() {
    return pht('%s Úng dụng', $this->getName());
  }
...
/* -(  Policies  )----------------------------------------------------------- */
  ...
  final private function getCustomCapabilitySpecification($capability) {
    ...
    if (!isset($custom[$capability])) {
      throw new Exception(pht("Không biết '%s'!", $capability));
    }
